...
namespace wafflepp
{
  ...
  struct Request
  {
    ...
    template <typename T>
    T getJSON(const std::string &key, const T &default_value)
    {
  ...
  struct Response
  {
    ...
    {
      return this->json()->body(std::move(str));
    }
    Response *json(const std::string &str)
    {
      return this->json()->body(str);
    }
    Response *html()
    {
      return this->content("text/html");
    }
    Response *html(std::string &&str)
    {
      return this->html()->body(std::move(str));
    }
    Response *html(const std::string &str)
    {
      return this->html()->body(str);
    }
    Response *abort(int status, const std::string &description = "")
    {
      ...
      std::cerr << "Abort: " << status << " " << description << std::endl;
  ...
  static std::unordered_map<std::string, std::unordered_map<std::string, URLCallback>> callbacks;
  ...
  public:
    ...
    void listen(http_server_s *server, int port)
    {
...
#define ADD_METHOD(method_name, method)               \
  ...
  {                                                   \
    ...
    /*full_url.append(".*");*/                        \
    wafflepp::callbacks[url][#method] = cb;           \
  }
